############################modified version 2##################################
#the difference between this version with the two versions above is that
#1. the dot represents sQTL exon instead of sQTL exon-SNP pair
#2. for each exon in each brain region, we go over all the significant SNPs, instead of using the actually distance (in bp), we classify it into different categories
#then we pick the best category based on this order: dinucleotide, splice site range, exon body, <=100bp, <=200bp, <=300bp, >300bp, i.e., if there is one on the dinucleotide range, this exon is considered this category no matter the rest of the 12 categories
#3. When we make the plot, each exon will have two information: 
#one category: we choose one from the 13 categories in this order: dinucleotide, splice site range, exon body, <=100bp, <=200bp, <=300bp, >300bp
#the number of significant regions
...
#######################################################################
#just the plot part (after getting the statistics from the code above)#The parameters are a little bit different from the above plots
#######################################################################
brainregionlist=c("Brain-Amygdala",
                  "Brain-AnteriorcingulatecortexBA24",
                  "Brain-Caudatebasalganglia",
...
#2. read in the statistics from the first part of the code
setwd("/u/nobackup/yxing/PROJECT/yidazhan/research/rotation_project/GTEx_brain_project_V7/analysis/6.2_sQTL_SNP_annotation/result/SNP_to_SS_distance_vs_brain_region_specificity/closest_significant_SNP_categorized_distance")
result=read.table(paste(splicetype,"_",type,"_exonclass.txt",sep=""),sep="\t",header=T)
exonclass=as.matrix(result[,1:length(brainregionlist)])
num.sig.region=result[,length(brainregionlist)+1]
...
p <- ggplot(df, aes(x=dis_2_SS, y=num.sig.region, fill=dis_2_SS)) + 
  ...
  #geom_jitter(shape=16, width=0.1, height=0) +    #jitter on the horizontal level
  #geom_violin(scale="width") + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + geom_boxplot(width=0.2) + geom_jitter(shape=16, position=position_jitter(0.05)) +    #jitter on both horizontal and vertical level
  labs(x = "distance to splice site", y="number of significant regions") + 
  scale_y_continuous(breaks = round(seq(0, 13, by = 1),1)) + 
  theme(
    # axis
...
#another violin plot (flip x and y axis)
df_flip=as.matrix(df)
#change characters to number (violin plot cannot apply to characters)
for (i in 1:length(importance_order)){
  df_flip[which(df_flip[,"dis_2_SS"] %in% importance_order[i]),"dis_2_SS"]=i
}
df_flip=as.data.frame(df_flip)
df_flip$dis_2_SS=as.numeric(df_flip$dis_2_SS)

setwd(outputpath)
pdf(paste(splicetype,type,"brain_region_specificity_vs_SNP_to_SS_distance_violin_plot.pdf"),height=6,width=6)
p <- ggplot(df_flip, aes(x=num.sig.region, y=dis_2_SS, fill=num.sig.region)) + 
  geom_violin(scale="width") + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + geom_boxplot(width=0.2) + 
  #geom_jitter(shape=16, width=0.1, height=0) +    #jitter on the horizontal level
  #geom_violin(scale="width") + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + geom_boxplot(width=0.2) + geom_jitter(shape=16, position=position_jitter(0.05)) +    #jitter on both horizontal and vertical level
  labs(x = "number of significant regions", y="distance to splice site") + 
  #scale_y_continuous(breaks = round(seq(0, length(importance_order), by = 1),1)) + 
  scale_y_continuous(breaks = round(seq(0, length(importance_order), by = 1)),labels=c(" ",importance_order)) + 
  theme(
    # axis
    axis.title.x = element_text(face="bold", size=16, margin = margin(t = 10, r = 0, b = 0, l = 0)),
    axis.title.y = element_text(face="bold", size=12, margin = margin(t = 0, r = 10, b = 0, l = 0), angle=90),
...
p=ggplot(cdfdf, aes(x=value)) + stat_ecdf(aes(colour=dis_2_SS)) + 
  ...
  scale_color_manual(values=c("#7FC97F", "#BEAED4", "#FDC086","#386CB0","#F0027F","#BF5B17","#666666")) + 
  #scale_color_npg() + 
  labs(x = "number of significant regions", y="") +
