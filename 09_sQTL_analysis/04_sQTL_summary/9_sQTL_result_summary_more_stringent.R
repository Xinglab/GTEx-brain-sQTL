#The difference of this code compared with sQTL_result_summary.R is that we restrict our result to transcriptome-wide significant GWAS SNPs
...
for (j in 1:2){
  ...
  for (i in 1:length(brainregion)){
    ...
    #remove rows with marginally significant GWAS SNPs (p value > 5e-8)
    LDresult=LDresult[which(LDresult[,2] %in% hc_GWAS[,"SNPS"]),]
    ...
    if (!(inherits(LDresult,"try-error"))){       #if the LDresult is not empty (there are significant sQTLs)
      ...
      #exon_fullI_ID  exon_short_ID genomic_position sQTL_rsID  +related_information  GWAS_high_LD_SNP_rsID  +related_information  related_to_disease_or_not high/low/no_significant_in_current_brain_region high/low/no_significant_in_age exon_length 
      ...
      #sQTL rsID -> GWAS rsID
      #output=subset(output,output[,"sQTL"] %in% LDresult[,"sQTL"])   #filter out sQTLs that don't have high LD GWAS loci
      ...
      #The number of unique exons in the output matrix is smaller than totalsqtlexonnum is because exons with SNPs without rsID are removed
    }
  }
    ...
    for (j in 1:dim(summary)[1]){       #for each exon-SNP-GWAS trio
      ...
      if (trio %in% triolist){          #if this exon-SNP-GWAS trio is already in the table, this means that we only need to put the brain region p value and significance information into that row
        ...
        newrow=c(temp[3],    #gene symbol
                 ...
                 strsplit(t2g[which(t2g[,"external_gene_name"]==temp[3]),"description"][1],split="\\[")[[1]][1],  #gene description  
